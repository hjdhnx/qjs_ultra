...
#if defined(__ANDROID__) || !defined(HAVE_GETENTROPY)
#  include <openssl/rand.h>
#endif
#if defined(APE)
// Actually Portable Executable
// All functions are already included in cosmopolitan.h
#elif defined(__wasi__) || defined(__APPLE__) || defined(__ANDROID_API__) || \
...
static inline const void* copy_iov_to_host(IM3Runtime runtime, void* _mem,
                                           struct iovec* host_iov,
                                           wasi_iovec_t* wasi_iov,
                                           int32_t iovs_len) {
...
m3ApiRawFunction(m3_wasi_generic_args_get) {
  m3ApiReturnType(uint32_t) m3ApiGetArgMem(uint32_t*, argv)
...
m3ApiRawFunction(m3_wasi_generic_args_sizes_get) {
  m3ApiReturnType(uint32_t) m3ApiGetArgMem(__wasi_size_t*, argc)
...
m3ApiRawFunction(m3_wasi_generic_environ_get) {
  m3ApiReturnType(uint32_t) m3ApiGetArgMem(uint32_t*, env)
...
m3ApiRawFunction(m3_wasi_generic_environ_sizes_get) {
  m3ApiReturnType(uint32_t) m3ApiGetArgMem(__wasi_size_t*, env_count)
...
m3ApiRawFunction(m3_wasi_generic_fd_prestat_dir_name) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(__wasi_fd_t, fd)
      m3ApiGetArgMem(char*, path) m3ApiGetArg(__wasi_size_t, path_len)
...
m3ApiRawFunction(m3_wasi_unstable_fd_seek) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(__wasi_fd_t, fd)
      m3ApiGetArg(__wasi_filedelta_t, offset) m3ApiGetArg(uint32_t, wasi_whence)
...
m3ApiRawFunction(m3_wasi_generic_path_open) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(__wasi_fd_t, dirfd)
      m3ApiGetArg(__wasi_lookupflags_t, dirflags)
          m3ApiGetArgMem(const char*, path) m3ApiGetArg(__wasi_size_t, path_len)
  ...
  int flags = ((oflags & __WASI_OFLAGS_CREAT) ? _O_CREAT : 0) |
              ...
              ((fs_flags & __WASI_FDFLAGS_APPEND) ? _O_APPEND : 0) | _O_BINARY;
...
m3ApiRawFunction(m3_wasi_generic_fd_read) {
  ...
  const void* mem_check =
      copy_iov_to_host(runtime, _mem, iovs, wasi_iovs, iovs_len);
...
m3ApiRawFunction(m3_wasi_generic_random_get) {
  m3ApiReturnType(uint32_t) m3ApiGetArgMem(uint8_t*, buf)
...
#else
        /* Android 或没有 getentropy 时用 OpenSSL RAND_bytes */
#       if defined(__ANDROID__) || !defined(HAVE_GETENTROPY)
            retlen = (RAND_bytes((unsigned char *)buf, (int)reqlen) == 1) ? (int)reqlen : -1;
#       else
            retlen = getentropy(buf, reqlen) < 0 ? -1 : (int)reqlen;
#       endif
#endif
...
m3ApiRawFunction(m3_wasi_generic_clock_res_get) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(__wasi_clockid_t, wasi_clk_id)
...
m3_wasi_context_t* m3_GetWasiContext() { return wasi_context; }
...
M3Result m3_LinkWASI(IM3Module module) {
  ...
  _(SuppressLookupFailure(m3_LinkRawFunction(module, "wasi_unstable", "fd_seek",
                                             "i(iIi*)",
                                             &m3_wasi_unstable_fd_seek)));
  _(SuppressLookupFailure(
      m3_LinkRawFunction(module, "wasi_snapshot_preview1", "fd_seek", "i(iIi*)",
                         &m3_wasi_snapshot_preview1_fd_seek)));
  //_ (SuppressLookupFailure (m3_LinkRawFunction (module, "wasi_unstable",
  //"fd_filestat_get",   "i(i*)",     &m3_wasi_unstable_fd_filestat_get))); _
  //(SuppressLookupFailure (m3_LinkRawFunction (module,
  //"wasi_snapshot_preview1", "fd_filestat_get",   "i(i*)",
  //&m3_wasi_snapshot_preview1_fd_filestat_get))); _ (SuppressLookupFailure
  //(m3_LinkRawFunction (module, "wasi_unstable",          "path_filestat_get",
  //"i(ii*i*)",  &m3_wasi_unstable_path_filestat_get))); _
  //(SuppressLookupFailure (m3_LinkRawFunction (module,
  //"wasi_snapshot_preview1", "path_filestat_get", "i(ii*i*)",
  //&m3_wasi_snapshot_preview1_path_filestat_get)));

  for (int i = 0; i < 2; i++) {
    const char* wasi = namespaces[i];

    _(SuppressLookupFailure(
        m3_LinkRawFunctionEx(module, wasi, "args_get", "i(**)",
                             &m3_wasi_generic_args_get, wasi_context)));
    _(SuppressLookupFailure(
        m3_LinkRawFunctionEx(module, wasi, "args_sizes_get", "i(**)",
                             &m3_wasi_generic_args_sizes_get, wasi_context)));
    _(SuppressLookupFailure(
        m3_LinkRawFunction(module, wasi, "clock_res_get", "i(i*)",
                           &m3_wasi_generic_clock_res_get)));
    _(SuppressLookupFailure(
        m3_LinkRawFunction(module, wasi, "clock_time_get", "i(iI*)",
                           &m3_wasi_generic_clock_time_get)));
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, wasi, "environ_get", "i(**)", &m3_wasi_generic_environ_get)));
    _(SuppressLookupFailure(
        m3_LinkRawFunction(module, wasi, "environ_sizes_get", "i(**)",
                           &m3_wasi_generic_environ_sizes_get)));
    ...
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, wasi, "fd_datasync", "i(i)", &m3_wasi_generic_fd_datasync)));
    _(SuppressLookupFailure(
    ...
    //(m3_LinkRawFunction (module, wasi, "fd_filestat_set_size", "i(iI)",   )));
    //_     (SuppressLookupFailure (m3_LinkRawFunction (module, wasi,
    //"fd_filestat_set_times","i(iIIi)", ))); _     (SuppressLookupFailure
    //(m3_LinkRawFunction (module, wasi, "fd_pread",             "i(i*iI*)",)));
    _(SuppressLookupFailure(
    ...
    //"fd_pwrite",            "i(i*iI*)",)));
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, wasi, "fd_read", "i(i*i*)", &m3_wasi_generic_fd_read)));
    //_     (SuppressLookupFailure (m3_LinkRawFunction (module, wasi,
    ...
    // wasi, "fd_tell",              "i(i*)",   )));
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, wasi, "fd_write", "i(i*i*)", &m3_wasi_generic_fd_write)));
    ...
    //_     (SuppressLookupFailure (m3_LinkRawFunction (module, wasi,
    //"poll_oneoff",          "i(**i*)", &m3_wasi_generic_poll_oneoff)));
    _(SuppressLookupFailure(
        m3_LinkRawFunctionEx(module, wasi, "proc_exit", "v(i)",
                             &m3_wasi_generic_proc_exit, wasi_context)));
    //_     (SuppressLookupFailure (m3_LinkRawFunction (module, wasi,
    //"proc_raise",           "i(i)",    )));
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, wasi, "random_get", "i(*i)", &m3_wasi_generic_random_get)));
    //_     (SuppressLookupFailure (m3_LinkRawFunction (module, wasi,
    //"sched_yield",          "i()",     )));
