...
M3Result m3_LinkTable(IM3Module io_module, IM3Table i_table) {
  ...
  if (info.elemTy != t_info.elemTy) {
    return m3Err_tableImportMissing;
  }

  // Imported table's initial size must be >= expected initial size
  if (info.curSize < t_info.curSize) {
    return m3Err_tableImportMissing;
  }

  // If expected max is d_m3MaxSaneTableSize (meaning "no max" in wasm3),
  // then any imported max is acceptable.
  // Otherwise, imported max must be <= expected max.
  if (t_info.maxSize != d_m3MaxSaneTableSize && info.maxSize > t_info.maxSize) {
    return m3Err_tableImportMissing;
  }
