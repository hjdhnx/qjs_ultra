...
/*
 * qjs_fs_module.c - 文件系统模块实现（中文说明）
 *
 * 提供 Node.js 风格的 fs API（readFile/writeFile/stat/mkdir 等）。
 */
...
static char *qjs_fs_path(JSContext *cx, char storage[QJS_MAX_PATH + 1],
...
static JSValue qjs_fs_encode(JSContext *cx,
    const qjs_buffer_encoding_t *encoding, qjs_str_t *str);
...
static qjs_fs_entry_t qjs_flags_table[] = {
    { qjs_str("a"),   O_APPEND | O_CREAT | O_WRONLY },
    { qjs_str("a+"),  O_APPEND | O_CREAT | O_RDWR },
    { qjs_str("as"),  O_APPEND | O_CREAT | O_SYNC | O_WRONLY },
    { qjs_str("as+"), O_APPEND | O_CREAT | O_RDWR | O_SYNC },
    { qjs_str("ax"),  O_APPEND | O_CREAT | O_EXCL | O_WRONLY },
    { qjs_str("ax+"), O_APPEND | O_CREAT | O_EXCL | O_RDWR },
    { qjs_str("r"),   O_RDONLY },
    { qjs_str("r+"),  O_RDWR },
    { qjs_str("rs+"), O_RDWR   | O_SYNC },
    { qjs_str("w"),   O_CREAT  | O_TRUNC | O_WRONLY },
    { qjs_str("w+"),  O_CREAT  | O_TRUNC | O_RDWR },
    { qjs_str("wx"),  O_CREAT  | O_TRUNC | O_EXCL | O_WRONLY },
    { qjs_str("wx+"), O_CREAT  | O_TRUNC | O_EXCL | O_RDWR },
    { qjs_null_str, 0 }
};
...
static const JSCFunctionListEntry qjs_fs_export[] = {
    JS_OBJECT_DEF("constants",
                  qjs_fs_constants,
                  qjs_nitems(qjs_fs_constants),
                  JS_PROP_CONFIGURABLE),
    JS_OBJECT_DEF("promises",
                  qjs_fs_promises,
                  qjs_nitems(qjs_fs_promises),
                  JS_PROP_CONFIGURABLE),
    ...
    JSValueConst *argv, int calltype)
{
    ...
    if (calltype == QJS_FS_CALLBACK) {
        if (argc > 0) {
            callback = argv[qjs_min(argc - 1, 2)];
        }
...
static JSValue
qjs_fs_fd_read(JSContext *cx, int fd, qjs_str_t *data)
{
    u_char   *p, *end, *start;
    size_t   size;
    ssize_t  n;
    ...
    qjs_ftw_flags_t flags, qjs_ftw_trace_t *parent)
{
    ...
    if (type == QJS_FTW_D || type == QJS_FTW_DP) {
        ...
        for ( ;; ) {
            ...
            path[base] = '/';
            memcpy(&path[base + 1], d_name, length + qjs_length("\0"));
    ...
    JSValueConst *argv, int calltype)
{
    ...
    char        target_buf[QJS_MAX_PATH + 1],
                path_buf[QJS_MAX_PATH + 1];
    ...
    if (calltype == QJS_FS_CALLBACK) {
        callback = argv[qjs_min(argc - 1, 3)];
...
process:
    ...
    if (calltype == QJS_FS_PROMISE) {
        ...
        if (JS_DefinePropertyValueStr(cx, result, "buffer", buffer,
                                      JS_PROP_C_W_E) < 0)
        {
            /* DefinePropertyValueStr 已消费 buffer，不再释放 */
            JS_FreeValue(cx, result);
            result = JS_EXCEPTION;
            goto done;
        }
    ...
    uint32_t *plen, JSValueConst obj)
{
    ...
    for (i = 0; i < qjs_nitems(stat_props); i++) {
...
qjs_fs_flags(JSContext *cx, JSValue value, int default_flags)
{
    ...
    for (fl = &qjs_flags_table[0]; fl->name.length != 0; fl++) {
        if (qjs_strstr_eq(&flags, &fl->name)) {
    ...
    const char *path, int errn)
{
    ...
    if (errn != 0) {
        ...
        if (JS_SetPropertyStr(cx, value, "code",
                              JS_NewString(cx, qjs_errno_string(errn))) < 0)
        {
            JS_FreeValue(cx, value);
            return JS_EXCEPTION;
        }
...
static char *
qjs_fs_path(JSContext *cx, char storage[QJS_MAX_PATH + 1], JSValue src,
    const char *prop_name)
{
    ...
    if (bytes.length > QJS_MAX_PATH - 1) {
    ...
    p = qjs_cpymem(storage, bytes.start, bytes.length);
    *p++ = '\0';
...
int qjs_fs_install_global(JSContext *cx, JSValueConst global)
{
    JSValue      proto;
                              ...
                              QJS_CORE_CLASS_ID_FS_STATS))
    {
        ...
        JS_SetPropertyFunctionList(cx, proto, qjs_fs_dirent_proto,
                                   qjs_nitems(qjs_fs_dirent_proto));
        ...
        JS_SetPropertyFunctionList(cx, proto, qjs_fs_filehandle_proto,
                                   qjs_nitems(qjs_fs_filehandle_proto));

        JS_SetClassProto(cx, QJS_CORE_CLASS_ID_FS_FILEHANDLE, proto);
    }

    /* ---------- 以下代码替换为挂载到全局 ---------- */
    JSValue fs = JS_NewObject(cx);
    if (JS_IsException(fs))
        return -1;

    JS_SetPropertyFunctionList(cx, fs, qjs_fs_export,
                                   qjs_nitems(qjs_fs_export));
                                   
    if (JS_DefinePropertyValueStr(cx, global,
                                  "fs",
                                  fs,
