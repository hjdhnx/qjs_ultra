...
/*
 * qjs_buffer.c - Buffer 模块实现（中文说明）
 *
 * 提供 Node.js 风格的 Buffer API，支持 ArrayBuffer/TypedArray 与 C 字节的互转。
 */

#include <qjs_native.h>
/* 字节序检测：小端平台上按主机字节序直接读写，大端平台交换字节。
 * 修复前 QJS_HAVE_LITTLE_ENDIAN 从未定义，LE/BE 读写语义在所有平台互换。 */
#ifndef QJS_HAVE_LITTLE_ENDIAN
#if defined(__BYTE_ORDER__) && defined(__ORDER_LITTLE_ENDIAN__) \
    && (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
#define QJS_HAVE_LITTLE_ENDIAN 1
#else
#define QJS_HAVE_LITTLE_ENDIAN 0
#endif
#endif
...
static const JSCFunctionListEntry qjs_buffer_export[] = {
    JS_OBJECT_DEF("constants",
                  ...
                  qjs_nitems(qjs_buffer_constants),
...
#ifndef QJS_HAVE_QUICKJS_NEW_TYPED_ARRAY
    ...
    JSValueConst *argv)
{
    ...
    for (i = 0; len != 0 && i < list_len; i++) {
        ...
        n = qjs_min((size_t) len, buf.length);
        p = qjs_cpymem(p, buf.start, n);
    ...
    JSValueConst encode, uint64_t offset, uint64_t end)
{
    ...
    {
        while (offset < end) {
            n = qjs_min(src.length, end - offset);
...
static JSValue
qjs_buffer_array_range(JSContext *ctx, qjs_str_t *array, JSValueConst start,
    ...
    JSValueConst source_start, JSValueConst source_end)
{
    ...
    size = qjs_min(src.length, target.length);
    ...
    JSValueConst *argv, int last)
{
    ...
    if (!JS_IsUndefined(argv[1])) {
        ...
        if (from >= 0) {
            from = qjs_min(from, length);

        } else {
            from = qjs_max(0, length + from);
        }
    }
...
encoding:
    ...
    if (last) {
        from = qjs_min(from, length - (int64_t) str.length);
    ...
    int argc, JSValueConst *argv, int magic)
{
    ...
    qjs_str_t       self;
    qjs_bool_t      little, swap;
    qjs_conv_f32_t  conv_f32;
    qjs_conv_f64_t  conv_f64;
    ...
    int argc, JSValueConst *argv, int magic)
{
    ...
    case 2:
        u32 = qjs_get_u16(&self.start[index]);

        if (swap) {
            u32 = qjs_bswap_u16(u32);
        }
    ...
    case 4:
        u32 = qjs_get_u32(&self.start[index]);
    ...
    int argc, JSValueConst *argv, int size)
{
    ...
    case 2:
        for (; p < end; p += 2) {
            qjs_set_u16(p, qjs_bswap_u16(qjs_get_u16(p)));
        }
    ...
    case 4:
        for (; p < end; p += 4) {
            qjs_set_u32(p, qjs_bswap_u32(qjs_get_u32(p)));
        }
    ...
    default:
        for (; p < end; p += 8) {
            qjs_set_u64(p, qjs_bswap_u64(qjs_get_u64(p)));
        }
    }
    ...
    int argc, JSValueConst *argv)
{
    ...
    /* Node 兼容：支持 toString(encoding, start, end) 范围参数（start 含、end 不含，
     * 负值从末尾计，默认 0 与 buffer.length）。 */
    if ((argc >= 2 && !JS_IsUndefined(argv[1])) ||
        (argc >= 3 && !JS_IsUndefined(argv[2]))) {
        uint64_t start = 0, end = src.length;
        if (argc >= 2 && !JS_IsUndefined(argv[1])) {
        ...
        if (argc >= 3 && !JS_IsUndefined(argv[2])) {
            int64_t e;
            if (JS_ToInt64(ctx, &e, argv[2])) {
                return JS_EXCEPTION;
            }
            end = (e < 0) ? (int64_t) src.length + e : (uint64_t) e;
        }
        if (start > src.length) {
            start = src.length;
        }
    ...
    if (qjs_slow_path(encoding == NULL)) {
        return JS_EXCEPTION;
    }
    ...
    if (qjs_slow_path(data.start == NULL)) {
...
write:
    ...
    max_length = qjs_min(max_length, src.length);
    ...
    if (encoding->decode == NULL) {
        ...
        while (p < end) {
            p = qjs_utf8_next(p, src.start + src.length);
            if (p <= end) {
                prev = p;
            }
        }
    ...
    int argc, JSValueConst *argv, int magic)
{
    ...
    case 2:
        ...
        qjs_set_u16(&self.start[index], u32);
        break;
    ...
    case 4:
        ...
        qjs_set_u32(&self.start[index], u32);
        break;
    ...
    int argc, JSValueConst *argv, int magic)
{
    ...
    case 4:
        ...
        if (swap) {
            conv_f32.u = qjs_bswap_u32(conv_f32.u);
        }
    ...
    default:
        ...
        if (swap) {
            conv_f64.u = qjs_bswap_u64(conv_f64.u);
        }
...
reject:
    ...
    for (i = 0; i < len; i++) {
        ...
        if (qjs_slow_path(JS_IsException(ret))) {
            return ret;
        }

        if (qjs_slow_path(JS_ToInt32(ctx, &v, ret))) {
            return JS_EXCEPTION;
        }
...
qjs_buffer_encoding(JSContext *ctx, JSValueConst value, JS_BOOL thrw)
{
    ...
    for (encoding = &qjs_buffer_encodings[0];
         ...
         encoding++)
    {
        if (qjs_strstr_eq(&name, &encoding->name)) {
...
static void
qjs_base64_encode_core(qjs_str_t *dst, const qjs_str_t *src,
    const u_char *basis, qjs_bool_t padding)
{
   u_char  *d, *s, c0, c1, c2;
   size_t  len;
...
int
qjs_base64_encode(JSContext *ctx, const qjs_str_t *src, qjs_str_t *dst)
{
...
size_t
qjs_base64_encode_length(JSContext *ctx, const qjs_str_t *src)
{
...
static void
qjs_base64_decode_core(qjs_str_t *dst, const qjs_str_t *src,
    const u_char *basis)
{
    size_t  len;
    u_char  *d, *s;
...
static size_t
qjs_base64_decode_length_core(const qjs_str_t *src, const u_char *basis)
{
    unsigned int pad;
    size_t  len;
...
int
qjs_base64_decode(JSContext *ctx, const qjs_str_t *src, qjs_str_t *dst)
{
...
size_t
qjs_base64_decode_length(JSContext *ctx, const qjs_str_t *src)
{
...
int
qjs_base64url_encode(JSContext *ctx, const qjs_str_t *src, qjs_str_t *dst)
{
...
int
qjs_base64url_decode(JSContext *ctx, const qjs_str_t *src, qjs_str_t *dst)
{
...
size_t
qjs_base64url_decode_length(JSContext *ctx, const qjs_str_t *src)
{
    return qjs_base64_decode_length_core(src, qjs_basis64url);
}


qjs_inline qjs_int_t
...
int
qjs_hex_decode(JSContext *ctx, const qjs_str_t *src, qjs_str_t *dst)
{
    u_char        *p;
    size_t        len;
    qjs_int_t     c;
...
size_t
qjs_hex_decode_length(JSContext *ctx, const qjs_str_t *src)
{
    const u_char  *p, *end;
    ...
    for (; p < end; p++) {
        if (qjs_slow_path(qjs_char_to_hex(*p) < 0)) {
            break;
        }
    }
...
int
qjs_hex_encode(JSContext *ctx, const qjs_str_t *src, qjs_str_t *dst)
{
    u_char        *p, c;
    size_t        i, len;
    const u_char  *start;
...
size_t
qjs_hex_encode_length(JSContext *ctx, const qjs_str_t *src)
{
    return src->length * 2;
}
...
#ifdef QJS_HAVE_QUICKJS_NEW_TYPED_ARRAY
