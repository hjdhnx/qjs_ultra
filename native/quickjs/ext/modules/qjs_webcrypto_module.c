...
static JSValue qjs_cipher_pkey(JSContext *cx, qjs_str_t *data,
    qjs_webcrypto_key_t *key, int encrypt);
static JSValue qjs_cipher_aes_gcm(JSContext *cx, qjs_str_t *data,
    qjs_webcrypto_key_t *key, JSValue options, int encrypt);
static JSValue qjs_cipher_aes_ctr(JSContext *cx, qjs_str_t *data,
    qjs_webcrypto_key_t *key, JSValue options, int encrypt);
static JSValue qjs_cipher_aes_cbc(JSContext *cx, qjs_str_t *data,
    qjs_webcrypto_key_t *key, JSValue options, int encrypt);
...
static qjs_str_t *qjs_algorithm_curve_name(int curve);
...
#define qjs_webcrypto_algorithm(type, usage, fmt, raw)                       \
    ...
    {
      qjs_str("RSA-PKCS1-v1_5"),
      qjs_webcrypto_algorithm(QJS_ALGORITHM_RSA_PKCS1_v1_5,
...
static qjs_webcrypto_entry_t qjs_webcrypto_hash[] = {
    { qjs_str("SHA-256"), QJS_HASH_SHA256 },
    { qjs_str("SHA-384"), QJS_HASH_SHA384 },
    { qjs_str("SHA-512"), QJS_HASH_SHA512 },
    { qjs_str("SHA-1"), QJS_HASH_SHA1 },
    { qjs_null_str, 0 }
};


static qjs_webcrypto_entry_t qjs_webcrypto_curve[] = {
    { qjs_str("P-256"), NID_X9_62_prime256v1 },
    { qjs_str("P-384"), NID_secp384r1 },
    { qjs_str("P-521"), NID_secp521r1 },
    { qjs_null_str, 0 }
};


static qjs_webcrypto_entry_t qjs_webcrypto_format[] = {
    { qjs_str("raw"), QJS_KEY_FORMAT_RAW },
    { qjs_str("pkcs8"), QJS_KEY_FORMAT_PKCS8 },
    { qjs_str("spki"), QJS_KEY_FORMAT_SPKI },
    { qjs_str("jwk"), QJS_KEY_FORMAT_JWK },
    { qjs_null_str, QJS_KEY_FORMAT_UNKNOWN }
};


static qjs_webcrypto_entry_t qjs_webcrypto_jwk_kty[] = {
    { qjs_str("RSA"), QJS_KEY_JWK_KTY_RSA },
    { qjs_str("EC"), QJS_KEY_JWK_KTY_EC },
    { qjs_str("oct"), QJS_KEY_JWK_KTY_OCT },
    { qjs_null_str, QJS_KEY_JWK_KTY_UNKNOWN }
};


static qjs_webcrypto_entry_t qjs_webcrypto_usage[] = {
    { qjs_str("decrypt"), QJS_KEY_USAGE_DECRYPT },
    { qjs_str("deriveBits"), QJS_KEY_USAGE_DERIVE_BITS },
    { qjs_str("deriveKey"), QJS_KEY_USAGE_DERIVE_KEY },
    { qjs_str("encrypt"), QJS_KEY_USAGE_ENCRYPT },
    { qjs_str("sign"), QJS_KEY_USAGE_SIGN },
    { qjs_str("unwrapKey"), QJS_KEY_USAGE_UNWRAP_KEY },
    { qjs_str("verify"), QJS_KEY_USAGE_VERIFY },
    { qjs_str("wrapKey"), QJS_KEY_USAGE_WRAP_KEY },
    { qjs_null_str, 0 }
};


static qjs_webcrypto_entry_t qjs_webcrypto_alg_hash[] = {
    { qjs_str("RS1"), QJS_HASH_SHA1 },
    { qjs_str("RS256"), QJS_HASH_SHA256 },
    { qjs_str("RS384"), QJS_HASH_SHA384 },
    { qjs_str("RS512"), QJS_HASH_SHA512 },
    { qjs_str("PS1"), QJS_HASH_SHA1 },
    { qjs_str("PS256"), QJS_HASH_SHA256 },
    { qjs_str("PS384"), QJS_HASH_SHA384 },
    { qjs_str("PS512"), QJS_HASH_SHA512 },
    { qjs_str("RSA-OAEP"), QJS_HASH_SHA1 },
    { qjs_str("RSA-OAEP-256"), QJS_HASH_SHA256 },
    { qjs_str("RSA-OAEP-384"), QJS_HASH_SHA384 },
    { qjs_str("RSA-OAEP-512"), QJS_HASH_SHA512 },
    { qjs_null_str, 0 }
};
...
static qjs_str_t
...

    {
        qjs_null_str,
        qjs_str("RSA-OAEP"),
        qjs_str("RSA-OAEP-256"),
        qjs_str("RSA-OAEP-384"),
        qjs_str("RSA-OAEP-512"),
    },
...
static qjs_str_t qjs_webcrypto_alg_aes_name[3][3 + 1] = {
    {
        qjs_str("A128GCM"),
        qjs_str("A192GCM"),
        qjs_str("A256GCM"),
...
static const JSCFunctionListEntry qjs_webcrypto_export[] = {
    ...
    JS_OBJECT_DEF("subtle",
                  ...
                  qjs_nitems(qjs_webcrypto_subtle),
                  JS_PROP_CONFIGURABLE),
};
    ...
    int argc, JSValueConst *argv, int encrypt)
{
    ...
    case QJS_ALGORITHM_RSA_PKCS1_v1_5:
...
static JSValue
qjs_cipher_pkey(JSContext *cx, qjs_str_t *data, qjs_webcrypto_key_t *key,
    int encrypt)
{
    int                     rc;
    u_char                  *dst;
    ...
    // 根据算法类型设置不同的填充方式
    if (key->alg->type == QJS_ALGORITHM_RSA_OAEP) {
        EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_OAEP_PADDING);
        EVP_PKEY_CTX_set_rsa_oaep_md(ctx, md);
        EVP_PKEY_CTX_set_rsa_mgf1_md(ctx, md);
    } else if (key->alg->type == QJS_ALGORITHM_RSA_PKCS1_v1_5) {
        EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_PADDING);
    } else {
        qjs_webcrypto_error(cx, "unsupported algorithm: %s",
                            qjs_algorithm_string(key->alg));
        ret = JS_EXCEPTION;
        goto fail;
    }
...
static JSValue
qjs_cipher_aes_gcm(JSContext *cx, qjs_str_t *data, qjs_webcrypto_key_t *key,
...
static BIGNUM *
qjs_bn_counter128(qjs_str_t *ctr, unsigned bits)
{
...
static JSValue
qjs_cipher_aes_ctr(JSContext *cx, qjs_str_t *data, qjs_webcrypto_key_t *key,
    JSValue options, int encrypt)
{
    int               len, len2;
    u_char            *dst;
...
static JSValue
qjs_cipher_aes_cbc(JSContext *cx, qjs_str_t *data, qjs_webcrypto_key_t *key,
    JSValue options, int encrypt)
{
...
qjs_export_base64url_bignum(JSContext *cx, const BIGNUM *v, size_t size)
{
    ...
    if (qjs_bn_bn2binpad(v, &buf[0], size) <= 0) {
        JS_ThrowInternalError(cx, "qjs_bn_bn2binpad() failed");
        return JS_EXCEPTION;
    }
...
qjs_export_jwk_rsa(JSContext *cx, qjs_webcrypto_key_t *key)
{
    ...
    rsa = qjs_pkey_get_rsa_key(key->u.a.pkey);

    qjs_rsa_get0_key(rsa, &n_bn, &e_bn, &d_bn);
    ...
    if (key->u.a.privat) {
        qjs_rsa_get0_factors(rsa, &p_bn, &q_bn);
        qjs_rsa_get0_ctr_params(rsa, &dp_bn, &dq_bn, &qi_bn);
...
qjs_export_jwk_ec(JSContext *cx, qjs_webcrypto_key_t *key)
{
    ...
    ec = qjs_pkey_get_ec_key(key->u.a.pkey);
    ...
    if (!qjs_ec_point_get_affine_coordinates(group, pub, x_bn, y_bn)) {
...
qjs_export_jwk_oct(JSContext *cx, qjs_webcrypto_key_t *key)
{
    ...
    qjs_assert(key->u.s.raw.start != NULL);
    ...
    int argc, JSValueConst *argv)
{
    ...
    case QJS_ALGORITHM_RSA_OAEP:
        ...
        wkey->u.a.privat = 1;
        wkey->usage = (alg->type == QJS_ALGORITHM_RSA_OAEP || alg->type == QJS_ALGORITHM_RSA_PKCS1_v1_5)
                        ? QJS_KEY_USAGE_DECRYPT
                        : QJS_KEY_USAGE_SIGN;
        ...
        if (qjs_pkey_up_ref(wkey->u.a.pkey) <= 0) {
        ...
        wkeypub->usage = (alg->type == QJS_ALGORITHM_RSA_OAEP || alg->type == QJS_ALGORITHM_RSA_PKCS1_v1_5)
                          ? QJS_KEY_USAGE_ENCRYPT
                          : QJS_KEY_USAGE_VERIFY;
        ...
        if (JS_SetPropertyStr(cx, obj, "privateKey", key) < 0) {
            key = JS_UNDEFINED;  /* SetPropertyStr 已消费 key */
            goto fail;
        }
        ...
        if (JS_SetPropertyStr(cx, obj, "publicKey", keypub) < 0) {
            keypub = JS_UNDEFINED;  /* SetPropertyStr 已消费 keypub */
            goto fail;
        }
...
fail0:
    ...
    if (!qjs_rsa_set0_key(rsa, n_bn, e_bn, NULL)) {
        BN_free(n_bn);
        BN_free(e_bn);
...
fail1:
    ...
    if (!qjs_rsa_set0_key(rsa, NULL, NULL, d_bn)) {
        BN_free(d_bn);
        qjs_webcrypto_error(cx, "RSA_set0_key() failed");
        goto fail;
    }
    ...
    if (!qjs_rsa_set0_factors(rsa, p_bn, q_bn)) {
        BN_free(p_bn);
        BN_free(q_bn);
    ...
    if (!qjs_rsa_set0_ctr_params(rsa, dp_bn, dq_bn, qi_bn)) {
        BN_free(dp_bn);
        BN_free(dq_bn);
        BN_free(qi_bn);
...
static EVP_PKEY *
qjs_import_raw_ec(JSContext *cx, qjs_str_t *data, qjs_webcrypto_key_t *key)
{
    EC_KEY          *ec;
    EVP_PKEY        *pkey;
    EC_POINT        *pub;
...
qjs_import_jwk_oct(JSContext *cx, JSValue jwk, qjs_webcrypto_key_t *key)
{
    ...
    static qjs_webcrypto_entry_t hashes[] = {
        { qjs_str("HS1"), QJS_HASH_SHA1 },
        { qjs_str("HS256"), QJS_HASH_SHA256 },
        { qjs_str("HS384"), QJS_HASH_SHA384 },
        { qjs_str("HS512"), QJS_HASH_SHA512 },
        { qjs_null_str, 0 }
    };
    ...
    JSValueConst *argv)
{
    ...
    case QJS_ALGORITHM_RSA_PKCS1_v1_5:
        ...
        if (wkey->u.a.privat) {
            mask = (alg->type == QJS_ALGORITHM_RSA_OAEP || alg->type == QJS_ALGORITHM_RSA_PKCS1_v1_5)
...
qjs_ec_rs_size(EVP_PKEY *pkey)
{
    ...
    ec_key = qjs_pkey_get_ec_key(pkey);
    if (ec_key == NULL) {
        return 0;
    }
    ...
    bits = qjs_ec_group_order_bits(ec_group);
    if (bits == 0) {
        return 0;
    }
    ...
    JSValueConst *argv, int verify)
{
    ...
    case QJS_ALGORITHM_ECDSA:
    default:
        mctx = qjs_evp_md_ctx_new();
        if (mctx == NULL) {
            qjs_webcrypto_error(cx, "qjs_evp_md_ctx_new() failed");
            goto fail;
        }
...
qjs_webcrypto_key_algorithm(JSContext *cx, JSValueConst this_val)
{
    ...
    case QJS_ALGORITHM_RSA_OAEP:
        ...
        qjs_assert(key->u.a.pkey != NULL);
        qjs_assert(EVP_PKEY_id(key->u.a.pkey) == EVP_PKEY_RSA);

        qjs_rsa_get0_key(qjs_pkey_get_rsa_key(key->u.a.pkey), &n_bn, &e_bn,
                         NULL);
        ...
        if (JS_DefinePropertyValueStr(cx, obj, "publicExponent", pe,
            ...
            < 0)
        {
            /* DefinePropertyValueStr 已消费 pe，不再释放 */
            JS_FreeValue(cx, obj);
            return JS_EXCEPTION;
        }
    ...
    case QJS_ALGORITHM_ECDH:
        ...
        qjs_assert(key->u.a.pkey != NULL);
        qjs_assert(EVP_PKEY_id(key->u.a.pkey) == EVP_PKEY_EC);

        group = EC_KEY_get0_group(qjs_pkey_get_ec_key(key->u.a.pkey));
...
qjs_key_format(JSContext *cx, JSValueConst value)
{
    ...
    for (e = &qjs_webcrypto_format[0]; e->name.length != 0; e++) {
        if (qjs_strstr_eq(&format, &e->name)) {
...
qjs_key_algorithm(JSContext *cx, JSValue options)
{
    ...
    for (e = &qjs_webcrypto_alg[0]; e->name.length != 0; e++) {
        if (qjs_strstr_case_eq(&a, &e->name)) {
...
qjs_algorithm_hash(JSContext *cx, JSValue options, qjs_webcrypto_hash_t *hash)
{
    ...
    if (JS_IsObject(options)) {
        ...
        /* WebCrypto allows hash to be specified as either a string
         * ("SHA-256") or an AlgorithmIdentifier object ({name: "SHA-256"}).
         * If v is an object, extract its "name" property. */
        if (JS_IsObject(v)) {
            JSValue name_val = JS_GetPropertyStr(cx, v, "name");
            JS_FreeValue(cx, v);
            v = name_val;
            if (JS_IsException(v)) {
                return JS_EXCEPTION;
            ...
            if (JS_IsUndefined(v)) {
                JS_ThrowTypeError(cx, "hash object missing \"name\" property");
                return JS_EXCEPTION;
            }
        }
...
/* 简化版qjs_vsprintf，只处理%s和普通文本 */
static u_char *simple_vsprintf(u_char *buf, u_char *end, const char *fmt, va_list args)
{
    u_char *p = buf;
    const char *f = fmt;
    const char *str;
...
qjs_webcrypto_error(JSContext *cx, const char *fmt, ...)
{
    ...
    const char     *data;
    unsigned long  n;
    u_char         errstr[QJS_MAX_ERROR_STR];

    last = &errstr[QJS_MAX_ERROR_STR];

    va_start(args, fmt);
    p = simple_vsprintf(errstr, last - 1, fmt, args);
    va_end(args);
...
/* 直接注册到 globalThis.crypto，返回 0/-1 */
int qjs_webcrypto_install_global(JSContext *cx, JSValueConst global_obj)
{
    JSRuntime *rt = JS_GetRuntime(cx);
    JSValue crypto_obj, proto;

    /* ---------- 1. 注册 WebCryptoKey 类 ---------- */
    if (!JS_IsRegisteredClass(rt, QJS_CORE_CLASS_ID_WEBCRYPTO_KEY)) {
        if (JS_NewClass(rt, QJS_CORE_CLASS_ID_WEBCRYPTO_KEY,
                        &qjs_webcrypto_key_class) < 0)
            return -1;
        ...
        JS_SetPropertyFunctionList(cx, proto,
                                   ...
                                   qjs_nitems(qjs_webcrypto_key_proto));
        JS_SetClassProto(cx, QJS_CORE_CLASS_ID_WEBCRYPTO_KEY, proto);
    }

    /* ---------- 2. 拿到或创建 globalThis.crypto ---------- */

    crypto_obj = JS_GetPropertyStr(cx, global_obj, "crypto");
    if (JS_IsUndefined(crypto_obj) || JS_IsNull(crypto_obj)) {
        /* 不存在则新建 */
        JS_FreeValue(cx, crypto_obj);
        crypto_obj = JS_NewObject(cx);
        if (JS_IsException(crypto_obj)) {
            return -1;
        }
        if (JS_SetPropertyStr(cx, global_obj, "crypto", crypto_obj) < 0) {
            JS_FreeValue(cx, crypto_obj);
            return -1;
        }
    }

    /* ---------- 3. 把 webcrypto API 追加到 crypto 上 ---------- */
    JS_SetPropertyFunctionList(cx, crypto_obj,
                               qjs_webcrypto_export,
                               qjs_nitems(qjs_webcrypto_export));
