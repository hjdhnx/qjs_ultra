/*
 * quickjs_wrapper.h - QuickJS JNI 包装器头文件
 *
 * 声明 QuickJSWrapper 类（JNI 宿主层）。
 * wasm3 环境由引擎层（rt->ext_data）持有，JNI 不直接管理。
 *
...
public:
    ...
    jobjectArray getKeys(JNIEnv *env, jlong this_obj) const;
    jboolean contains(JNIEnv *env, jlong this_obj, jstring key) const;
    void arrayAdd(JNIEnv *env, jobject thiz, jlong this_obj, jobject value);
